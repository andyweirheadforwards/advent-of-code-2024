...
fun ReportStatusList.countSafeReports() = this.filter { it.isSafe() }.size

fun ReportList.totalSafeReports() = this.map { it.status() }.countSafeReports()

fun ReportList.totalSafeReportsWithDampener() =
    this.map { it.statusWithDampener() }.countSafeReports()

fun Report.getReportWithStatus(): ReportWithStatus {
  val levels = this.split(" ").map { it.toInt() }.listIterator()
...
private fun isValidStep(isIncreasing: Boolean?, current: Int, previous: Int): Boolean =
    when {
