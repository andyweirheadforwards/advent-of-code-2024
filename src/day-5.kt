...
fun main() {
    ...
    val updates: ManualUpdatesList = input.trim().lines()
        .takeLastWhile { it.isNotBlank() }
        .joinToString("\n").toManualUpdatesList()

    val sumOfMiddlePages: Int = updates.filterCorrectUpdates(rules).sumOfMiddlePages()
    val sumOfCorrectedMiddlePages: Int = updates.filterIncorrectUpdates(rules).map { it.fix(rules) }.sumOfMiddlePages()
...
fun ManualUpdatesList.filterCorrectUpdates(rules: RuleList): ManualUpdatesList = filter { it.isCorrect(rules) }

fun ManualUpdatesList.filterIncorrectUpdates(rules: RuleList): ManualUpdatesList = filter { !it.isCorrect(rules) }
...
fun RuleListString.toRuleList(): RuleList = trim().lines()
