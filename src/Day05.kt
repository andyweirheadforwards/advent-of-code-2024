...
fun main() {
  ...
  val rules: RuleList =
      input.trim().lines().takeWhile { it.isNotBlank() }.joinToString("\n").toRuleList()
...
fun ManualUpdatesString.toManualUpdatesList() =
    trim().lines().map { it.split(',').map { it.trim().toInt() } }
...
fun RuleListString.toRuleList(): RuleList =
    trim().lines().map { it.split('|').map { it.toInt() } }.map { Rule(it.first(), it.last()) }
