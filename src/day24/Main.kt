...
fun generateCombinations(
    ...
    pairCount: Int,
): Sequence<Set<Pair<LogicGate, LogicGate>>> =
    sequence {
        val n = gates.size
...
fun String.getWires(): Map<String, Wire> =
    this.trim().lines().takeWhile { it.isNotBlank() }.associate { line ->
        val (name, value) = line.split(":").map { it.trim() }
        name to Wire(name, value.toInt())
    } // Use toMap() instead of associate for better performance

fun String.getLogicGates(): List<LogicGate> =
    this.trim().lines().takeLastWhile { it.isNotBlank() }.map { line ->

        val (_, one, type, two, out) =
