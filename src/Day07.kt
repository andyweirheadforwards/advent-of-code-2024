...
data class Calibration(val target: Long, val values: List<Int>) {
    ...
    val isValidOne: Boolean get() = calculateNextOne()
    val isValidTwo: Boolean get() = calculateNextTwo()
...
fun String.toCalibration(): Calibration = Calibration(
    takeWhile { it != ':' }.toLong(),
...
val CalibrationList.calibrationTotal: Long get() = sumOf { it.target }

val CalibrationList.validCalibrationTotalOne: Long get() = filter { it.isValidOne }.calibrationTotal
val CalibrationList.validCalibrationTotalTwo: Long get() = filter { it.isValidTwo }.calibrationTotal